...
def face_detection(image,model):
    ...
    preprocessed_input_image  = cv2.dnn.blobFromImage(cv2.resize(image,(300,300)),scalefactor=1.0,size=(300,300),mean=(104.0, 177.0, 123.0))
    ...
    for face in detections[0][0]:        
        ...
        if face_confidence > 0.5:#min_confidence=0.5
            ...
            bbox = face[3:]
            print(bbox)#it give us the (4,)shape array value.
            ...
            cv2.rectangle(image, pt1=(x1, y1), pt2=(x2, y2), color=(0, 255, 0), thickness=image_width//200)
            ...
            cv2.rectangle(image, pt1=(x1, y1-image_width//20), pt2=(x1+image_width//16, y1),
                              color=(0, 255, 0), thickness=1)

            # Write the confidence score of the face near the bounding box and on the filled rectangle. 
            cv2.putText(image, text=str(round(face_confidence, 1)), org=(x1, y1-25), 
...
#load image
image = cv2.imread(r"C:\Users\user\Desktop\AAmer works\test1.jpg")
...
#calling funtion
face_detection(image,opencv_dnn_model)
